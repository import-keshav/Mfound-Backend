...
module.exports = {
       verifyLoginOTP : (id,mobile,otp, callback)=>{
		let query = "SELECT * FROM users WHERE id = "+id+" and mobile =  '"+mobile+"'";
        ...
        checkMobileUser : (validParams,callback)=>{
		let q = "SELECT * FROM users WHERE mobile = '"+validParams.mobile+"'  ";
		db.query(q,(err,rows)=>{console.log(err);console.log(rows);
			if(err){
				callback(commonFunction.callbackResponse(500,err));		
			}else{
				if(rows.length == 1){
	...
	checkUserCallback : (validParams,callback)=>{
		...
		db.query(q,(err,rows)=>{console.log(err);
			if(err){
				callback(commonFunction.callbackResponse(500,err));		
			}else{
				if(rows.length == 1){
	...
	sendSignupEmail  : (mailParams , callback)=>{
        ...
        sendFoundEmail  : (mailParams , callback)=>{
                mailServer.sendFoundEmail(mailParams, (mailResponse)=>{
                        if(mailResponse && mailResponse.ErrorCode == 0){
                                callback(commonFunction.callbackResponse(200,[]));                      
                        }else{
                              	callback(commonFunction.callbackResponse(503,[]));                      
	...
	sendSignupEmailRegistered : (mailParams , callback)=>{
       ...
       sendFoundSMS : (smsparams , callback)=>{
                smsServer.sendFoundSMS(smsparams,(smsResponse)=>{console.log(smsResponse);
                       callback(commonFunction.callbackResponse(200,[]));
                       // if(smsResponse.sid){
                       //       	callback(commonFunction.callbackResponse(200,[]));                      
                       // }else{
	...
	sendOTPSMS : (smsparams , callback)=>{
		smsServer.sendOTPSMS(smsparams,(smsResponse)=>{console.log(smsResponse);
                       callback(commonFunction.callbackResponse(200,[]));
			//if(smsResponse.sid){
			//	callback(commonFunction.callbackResponse(200,[]));			
			//}else{
	...
	verifyOTP : (id,mobile,otp, callback)=>{
		let query = "SELECT otp FROM users WHERE id = "+id+"";
		db.query(query,(err,rows)=>{
			if(err){
				callback(commonFunction.callbackResponse(500,err));		
			}else{
	...
	// 				callback(commonFunction.callbackResponse(200,rows[0]));	
	// 			}else{
	// 				let q2 = "select * from users where  email = '"+params.email+"'";
	...
	// 					}else{
	// 						if(rows2.length == 1){
	// 							callback(commonFunction.callbackResponse(200,rows2[0]));	
	// 						}else{
	// 							let whereCond4 = "";
	// 							(params.fb_id) ? (whereCond4 = "fb_id = '"+params.fb_id+"'") :  (whereCond4 = "device_token = '"+params.device_token+"'")
								
	// 							let q4 = "select * from users where  "+whereCond4+"";
	...
	// 								}else{
	// 									if(rows4.length == 1){
	// 										callback(commonFunction.callbackResponse(200,rows4[0]));	
	// 									}else{
	// 										callback(commonFunction.callbackResponse(404,[]));			
	// 									}
	// 								}
