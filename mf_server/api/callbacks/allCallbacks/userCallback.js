...
module.exports = {
	getUserDetail : (id , callback)=>{
		let q = "SELECT first_name , last_name , middle_name , email , mobile , gender , dob , address , city , state , zip , country , image , is_email_verified , wallet,  is_mobile_verified , is_active FROM users WHERE  id =  "+id+" ";
		db.query(q , (err,rows)=>{
			if(err){
